import mysql.connector

def cria_csvs(resultado):
    resultado['data_frame_tratado'].to_csv('dados/fato_produtos.csv', sep=';', index=False)
    resultado['dim_produtos'].to_csv('dados/dim_produtos.csv', sep=';', index=False)
    resultado['dim_categorias'].to_csv('dados/dim_categorias.csv', sep=';', index=False)
    resultado['dim_marcas'].to_csv('dados/dim_marcas.csv', sep=';', index=False)

def conectar_banco():
    conexao = mysql.connector.connect(host='localhost', database='teste', user='root', password='8676')

    if conexao.is_connected():
        print('[LOAD] Conectado ao banco de dados')
        cursor = conexao.cursor()

    return {'conexao' : conexao, 
            'cursor' : cursor}

def criar_tables(banco_de_dados):
    banco_de_dados['cursor'].execute('''CREATE TABLE IF NOT EXISTS dim_produtos (
                   produto_id INT PRIMARY KEY,
                   nome VARCHAR(100),
                   descricao TEXT)''')
    print('[LOAD] Criada table dim_produto')

    banco_de_dados['cursor'].execute('''CREATE TABLE IF NOT EXISTS dim_categorias (
                   categoria_id INT PRIMARY KEY,
                   categoria VARCHAR(50))''')
    print('[LOAD] Criada table dim_categoria')

    banco_de_dados['cursor'].execute('''CREATE TABLE IF NOT EXISTS dim_marcas (
                   marca_id INT PRIMARY KEY,
                   marca VARCHAR(50))''')
    print('[LOAD] Criada table dim_marcas')

    banco_de_dados['cursor'].execute('''CREATE TABLE IF NOT EXISTS fato_produtos (
                   id INT PRIMARY KEY,
                   preco DECIMAL(10,2),
                   nota DECIMAL(4,2),
                   estoque INT,
                   valor_total_estoque DECIMAL(10,2),
                   score_produto DECIMAL(10,2),
                   nivel_estoque VARCHAR(50),
                   nivel_avaliacao VARCHAR(50),
                   preco_relativo VARCHAR(50),
                   status_score VARCHAR(50),

                   produto_id INT,
                   categoria_id INT,
                   marca_id INT,

                   FOREIGN KEY (produto_id) REFERENCES dim_produtos(produto_id),
                   FOREIGN KEY (categoria_id) REFERENCES dim_categorias(categoria_id),
                   FOREIGN KEY (marca_id) REFERENCES dim_marcas(marca_id))''')
    
    banco_de_dados['conexao'].commit()
    print('[LOAD] Criada table fato_produtos')

def insert_dim_produtos(banco_de_dados, resultado):
    banco_de_dados['cursor'].executemany('''INSERT IGNORE INTO dim_produtos (produto_id, nome, descricao)
                   VALUES (%s, %s, %s)''', resultado['lista_dim_produtos'])
    banco_de_dados['conexao'].commit()
    print('[LOAD] Insert realizado em: dim_produtos')

def insert_dim_categorias(banco_de_dados, resultado):
    banco_de_dados['cursor'].executemany('''INSERT IGNORE INTO dim_categorias (categoria_id, categoria)
                       VALUES (%s, %s)''', resultado['lista_dim_categorias'])
    banco_de_dados['conexao'].commit()
    print('[LOAD] Insert realizado em: dim_categorias')

def insert_dim_marcas(banco_de_dados, resultado):
    banco_de_dados['cursor'].executemany('''INSERT IGNORE INTO dim_marcas (marca_id, marca)
                       VALUES (%s, %s)''', resultado['lista_dim_marcas'])
    banco_de_dados['conexao'].commit()
    print('[LOAD] Insert realizado em: dim_marcas')

def insert_fato_produtos(banco_de_dados, resultado):
    banco_de_dados['cursor'].executemany('''INSERT IGNORE INTO fato_produtos
                       (id, preco, nota, estoque, valor_total_estoque, score_produto,
                       nivel_estoque, nivel_avaliacao, preco_relativo, status_score,
                       produto_id, categoria_id, marca_id)
                       VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)''', resultado['lista_produtos'])
    banco_de_dados['conexao'].commit()
    print('[LOAD] Insert realizado em: fato_produtos')

def criar_base_de_dados(resultado):
    cria_csvs(resultado)

    banco_de_dados = conectar_banco()

    criar_tables(banco_de_dados)

    insert_dim_produtos(banco_de_dados, resultado)
    insert_dim_categorias(banco_de_dados, resultado)
    insert_dim_marcas(banco_de_dados, resultado)
    insert_fato_produtos(banco_de_dados, resultado)

    banco_de_dados['cursor'].close()
    banco_de_dados['conexao'].close()